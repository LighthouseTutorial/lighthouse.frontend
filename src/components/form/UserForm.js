...
export default class UserForm extends React.Component
{
    ...
    componentDidMount()
    {
        if (this.props.user)
        {
            const { id, email, coins } = this.props.user
            this.setState({ id, email, coins });
        }
    }
    onChange = e =>
    {
    ...
    async updateUser()
    {
        ...
        fetch(`${window.USERS_API_URL}/${this.state.id}`, {
            ...
            body: JSON.stringify({
                ...
                coins: parseFloat(this.state.coins)
            })
        })
            .then(async response =>
            {
    ...
    render()
    {
        return <Form noValidate validated={this.state.validated} onSubmit={this.submitEdit}>
            ...
            <Form.Group>
                ...
                <Form.Control type="number" name="coins" onChange={this.onChange} value={this.state.coins} required />
